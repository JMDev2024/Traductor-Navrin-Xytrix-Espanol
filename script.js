...
// Inicializamos meSpeak
meSpeak.loadConfig("mespeak_config.json"); // ruta al config
meSpeak.loadVoice("voices/es.json");       // ruta a la voz en español

let lastAudioData = null; // Para guardar audio generado

document.getElementById("speakBtn").addEventListener("click", () => {
    const text = document.getElementById("outputPhonetic").value;
    if (!text.trim()) {
    ...
    // Reproducir el texto
    meSpeak.speak(text, { amplitude: 100, pitch: 50, speed: 150, wordgap: 0 });

    // Generar audio en memoria (ArrayBuffer)
    lastAudioData = meSpeak.speak(text, { rawdata: "arraybuffer", amplitude: 100, pitch: 50, speed: 150, wordgap: 0 });
});

document.getElementById("downloadBtn").addEventListener("click", () => {
    if (!lastAudioData) {
        alert("Primero genera el audio haciendo clic en 'Hablar'.");
        return;
    }

    // Crear Blob y enlace para descarga
    const blob = new Blob([lastAudioData], { type: "audio/wav" });
